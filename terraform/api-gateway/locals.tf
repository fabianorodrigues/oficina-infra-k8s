locals {
  ...
  backend_listener_ssm_parameter_name = length(trimspace(var.backend_listener_ssm_parameter_name)) > 0 ? var.backend_listener_ssm_parameter_name : "/oficina/${var.environment}/api/backend-listener-arn"
  public_base_url_ssm_parameter_name  = "/oficina/${var.environment}/api/public-base-url"
